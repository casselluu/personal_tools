...
#    Copyright (C) 1999 - 2010 K.H. Wolfe, K. Hokamp
#
...
my $version_number = '$Revision: 2.36 $'; # '  $Date: 2016/09/10 13:25:24 $
...
my $sender = 'pubcrawlerhelp@gmail.com';     # <- PLEASE change to YOUR address
#my $sender = 'pubcrawlerinfo@gmail.com';     # <- PLEASE change to YOUR address
...
# PLEASE adjust following to match your web server setup
# See also pubcrawler_www.pl
my $HTTPD = 'http://pubcrawler.gen.tcd.ie/';
my $CGI = $HTTPD . 'cgi-bin/';
my $HTML = $HTTPD . '';
my $PICS = $HTTPD . 'pics/';
my $link_gen = $HTML;
my $home_link = 'http://pubcrawler.gen.tcd.ie';
...
my %ncbi_options = ('pubmed', 'DocSum, Brief, Abstract, AbstractPlus, Citation, MEDLINE, XML, ASN1, ExternalLink',
...
# set defaults:
$PARAM{'time_out'} = 60;
$PARAM{'test_URL'} = 'https://www.ncbi.nlm.nih.gov/';
$PARAM{'retry'} = 0;          # retry in case of 'Server Error'
$PARAM{'break'} = '20';           # seconds of sleep between each request
$PARAM{'pic'} = $PICS . 'pubcrawler_logo_new.jpg';
my $alias_global = '';
my $retrieve_URL_def = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi';
my $search_URL_def = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi';
#my $neighbour_URL_def = 'https://www.ncbi.nlm.nih.gov/entrez/utils/pmneighbor.fcgi';
my $neighbour_URL_def = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi';
my $relentrez_date_default = 180;
my %abstract_link = ('pubmed', 'https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Retrieve&dopt=Abstract&db=PubMed&list_uids=',
                     'nucleotide', 'https://www.ncbi.nlm.nih.gov/entrez/viewer.fcgi?db=nucleotide&val=');
my %checkbox_name = ('pubmed', '<INPUT TYPE="CHECKBOX" NAME="PubMedHits" VALUE=',
 ...
 # see https://www.ncbi.nlm.nih.gov/books/bv.fcgi?rid=helplinks.chapter.linkshelp for info on the next links:
my $summary_link = 'https://www.ncbi.nlm.nih.gov/sites/entrez?cmd=retrieve&dopt=Summary';
my $summary_link_pubmed = 'https://www.ncbi.nlm.nih.gov/pubmed/';
my $related_article_link = 'https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?db=PubMed&cmd=Display&dopt=pubmed_pubmed&from_uid=';
...
my $ncbi_form = '<form enctype="application/x-www-form-urlencoded" name="frmQueryBox" action="https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?SUBMIT=y" method="POST" onSubmit="if(inGo!=true){document.frmQueryBox.cmd.value=\'\';}">';
...
A:visited  { color: #5533FF; font-weight: bold; text-decoration: none }
...
function id_collect_ncbi(form)
{
  var allEmpty = true;
  var first = true;
  var url = "https://www.ncbi.nlm.nih.gov/entrez/query.fcgi";
...
function setFormat(choice) {
    ...
    switch (choice.value) {
        case "pubmed" :
	    var formatList = new Array("Abstract", "AbstractPlus", "DocSum", "Brief", "Citation", "MEDLINE", "XML", "ASN1", "ExternalLink")
	    break
        case "nucleotide" :
...
function ncbi_query(form) {
    ...
    var isPreNN6 = (navigator.appName == "Netscape" && parseInt(navigator.appVersion) <= 4);
    var allEmpty = true;
    var url = "https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=search&db=";
    var db_list = form.db;
    var db = db_list.options[db_list.selectedIndex].value;
...
GetOptions('add_path', \$PARAM{'add_path'},  # this option is dealt with in BEGIN{}
	   ...
	   'lynx=s', \$PARAM{'lynx'},
           'link_format=s', \$PARAM{'link_format'},
...
my @formats = ('Summary','DocSum','Brief','Abstract','AbstractPlus', 'Citation','MEDLINE','ASN.1','ExternalLink','GenBank','FASTA', 'XML');
...
my $spacer_gif = $PARAM{'spacer'} || $PICS . 'spacer.gif';
...
unless ($PARAM{'verbose'}) {        
    ...
    # change permissions, in case this is run through web server
    chmod (0666,"$PARAM{'log_file'}");
...
if (-e "$PARAM{'html_file'}.bak.html") {
    unlink "$PARAM{'html_file'}.bak.html";
}

my $backup_file = '';
if (-e "$PARAM{'html_file'}") {
    unless ($no_copy) {
        copy("$PARAM{'html_file'}","$PARAM{'html_file'}.bak.html");
        # change permissions, in case this is run through web server
        chmod (0666,"$PARAM{'html_file'}.bak.html");
        $backup_file = basename($PARAM{'html_file'}).'.bak.html';
    }
}
...
foreach $query (@query_order) {
	...
	if ($PARAM{'break'} < 1) {
	    select(undef, undef, undef, $PARAM{'break'});
        } else {
            sleep($PARAM{'break'});
        }
    }
...
for (my $i = 0; $i <= $#alias_order; $i++) {
	...
	if (@getmax_warning) {
	    ...
	    if ($PARAM{'quickstart'}) {
		push @status_message, "<B>-\&gt<A HREF=\"${HTML}quickstart.html\">Restart your jobs manually\!</A></B>";
                my $text_insert = "<A HREF=\"${HTML}quickstart.html\">Restart your jobs manually\!</A>";
                unless ($mail_features{'html'}) {
                    $text_insert = "Restart your jobs manually at ${HTML}quickstart.html";
                }
...
unless ($mail_features{'text'}) {
    ...
    my $mail_scripting_start = "\n$javascript\n<FORM NAME=\"myForm\" ACTION=\"https://www.ncbi.nlm.nih.gov/entrez/query.fcgi$PARAM{'extra_query'}\" method=\"POST\">";
            ...
            <td>
             <table cellpadding=\"0\" cellspacing=\"0\" class=\"pub\" border=\"1\" width=\"102\">
              <tr>
               <td>
                <div align=\"center\">
                 <p><A HREF=\"$home_link\"><font color=\"#000063\" size=\"7\"><b>Pub</b><br>
...
";
    ...
    if ($PARAM{'pic'}) {
	$picture = "<A HREF=\"$home_link\"><IMG BORDER=0 SRC=\"$PARAM{'pic'}\" ALT=\"LOGO\"></A>";
    } else {
	$picture = $picture_replacement;
    }
    
    my %user_links = ('FAQ', "${HTML}pubcrawler_www_faq.html",
#		      'News', "${HTML}webservice_news.html",
		      'Settings', "${CGI}pubcrawler_www.pl?submit=Log+in!&pc_user=$PARAM{'id'}",
#		      'Queries', "${CGI}pubcrawler_buttons?submit=Modify+Queries&pc_name=$PARAM{'id'}",
		      'WWW-Service', $HTML,
		      'Previous&nbsp;Results', "${CGI}pubcrawler_buttons?submit=Previous+Results&pc_name=$PARAM{'id'}",
		      'News', 'http://groups.google.com/group/pubcrawler-announcements',
		      );
...
<img SRC=\"${PICS}plos.jpg\" ALT=\"PLOS\"></a>
</CENTER>
</P>
";
...
left column
-->
    <td width=\"155\" bgcolor=\"#9ccfff\" valign=\"top\">    
     <div align=\"center\">
     ...
     <b><big>PubCrawler Results $username</big></b><br>
	  ...
	  $picture
          <div class=\"small\">
           ...
           &nbsp;<i>It goes to the library <br>
            - you go to the pub(TM)</i></font></div>
          </div>
         </td>
        </tr>
        <tr>
...
Execution of your PubCrawler jobs is currently in progress (started at <B>".(localtime)."</B>) ... please check again later!<BR>So far: approx. $update_message";
	...
	my $form = "<FORM NAME=\"myForm\" ACTION=\"https://www.ncbi.nlm.nih.gov/entrez/query.fcgi$PARAM{'extra_query'}\" method=\"POST\">";
	unless ($mail_features{'javascript'}) {
	    $form = '';
	}
...
sub compile_notification {
	...
	# if we still have a single quote within the address,
	# make sure it is escaped and put address into double quotes:
	if ($address_clean =~ /\'/) {
...
Number of new hits: $total_hits\n\n";
    ...
    if ($mail_features{'pubcrawler_links'}) {
	my $link = "<a href=\"${CGI}pubcrawler.results\">${CGI}pubcrawler.results</a>";
	unless ($mail_features{'html'}) {
            $link = "${CGI}pubcrawler.results";
	}
	print NOTE "${break}${break}If you have ${boldIn}'Easy Check'${boldOut} activated, you can see PubCrawler's output at $link.";
	$link = "<a href=\"${HTML}pubcrawler_www_faq.html\">FAQ</A>";
	unless ($mail_features{'html'}) {
	    $link = "FAQ at ${HTML}pubcrawler_www_faq.html";
	}
...
sub mail_service {
    ...
    my $subject = "PubCrawler Results, $subj_date$check_string ($total_hits hits)";
	...
	$subject = "PubCrawler Notification, $subj_date$check_string ($total_hits hits)";
	...
	if ($copy_mail_file) {
		...
		$mail_file = $PARAM{'html_file'}.'.mail';
		# change permissions, in case this is run through web server
		chmod (0666,"$mail_file");
		unless (open (OUT, ">$mail_file")) {
		    warn "Can't write mail to $mail_file: $!\n";
		    $mail_file = $PARAM{'html_file'};
		} else {
		    my $address_clean = $address2;
		    ($address_clean) = split /\#/, $address2 if ($address2 =~ /\#/);
		    ($address_clean) = split /\@\@/, $address2 if ($address2 =~ /\@\@/);
...
sub mail_results {
	...
	$err_txt .= "<BR><BR>\nYou should check your result at ${HTTPD}/db/${for_name} and consider restarting them manually.\n";
...
#				'retmax='.$PARAM{'getmax'}, # doesn't work!
    ...
    ($uid_result,$connection_error) = &make_HTTP($query_URL,$docstring, $query);
    ...
    while ( ( $uid_result =~ /Server Error/s
	      ...
	      $connection_error
	      or
	      $uid_result !~ /<eSearchResult>/si
	      or
	      ($uid_result !~ /<idlist>/si
	       and
	       $uid_result !~ /<eLinkResult>/si)
	     )
	    and
	...
	last if ($uid_result =~ /<eSearchResult>/si);
	my $text = ($connection_error ?
		    'Connection error' :
		    'Server error');
	$retry_local++;
	my $time_now = localtime;
	print "\nWARNING at $time_now on $ip: $text, starting retry $retry_local...\n";
	warn "WARNING at $time_now $PARAM{'id'} on $ip: $text, retry $retry_local for query $query at first visit\n" if ($retry_local > 3);
        print "sleeping for $PARAM{'break'} seconds...\n";
        sleep($PARAM{'break'} + 1);	
	$time_1 = time if ($PARAM{'log_queries'});
	($uid_result,$connection_error) = &make_HTTP($query_URL,$docstring, $query);
...
#    &update_output;
    ...
    if ($uid_result =~ /<eSearchResult><Count>0<\/Count>/is) {
	$uid_result = '';
    }
    if ($uid_result =~ /<eSearchResult>\n\s*<Count>0<\/Count>/is) {
	$uid_result = '';
    }
...
sub format_record {
    ...
    my @records = split /\n/, $record;
    while (@records) {
	my $line = shift @records;
	chomp $line;
	if ($line =~ /<Item Name/) {	
	    unless ($line =~ /\>$/) {
		until ($line =~ /<\/Item>/) {
		    last unless (@records);
		    my $line2 = shift @records;
		    chomp $line2;
		    $line .= " $line2";
		}
	    }
...
#	    if (defined @{$record{'CollectiveName'}}) {
	...
	# first line:	
		...
		$out[0] = join ', ', @{$record{'CollectiveName'}};
...
#between age 0 and age $PARAM{'viewdays'}:
	...
	my $prefix = "$summary_link\&db=$retrieve_db{$search_type}\&dispmax=200\&list_uids=";
	my $suffix = '';
	if ($search_type =~ /pubmed/i) {
	    $prefix = $summary_link_pubmed;
	    $suffix = '?report='.$PARAM{'link_format'}.'&dispmax=200&tool='.$PARAM{'tool'};
	}

	if ($day == 0) {
	        # assemble $PARAM{'fullmax'} of uids to uid string

	    # update Fri May 30 13:33:20 IST 2008 by Karsten Hokamp:
	    # if link is to PubMed, make it less than 4080 bytes long (including prefix and suffix)
...
#	    my $uid_tmp_lst = '';
#	    @query_list_1day = ();
#	    while (length($prefix.$uid_tmp_lst.$suffix) <= 4072) {
#		last unless (@query_big);
#		my $tmp = shift @query_big;
#		$uid_tmp_lst .= "$tmp,";
#		push @query_list_1day, $tmp;
#	    }

#	    $dispmax = 200;
#	    $counter = scalar @query_list_1day + 1;
...
#	    $counter = $PARAM{'extra_range'} + 1;
	    ...
	    while (length($prefix.$uid_tmp_lst.$suffix) <= 4072
		   and
		   @query_list_1day < $PARAM{'extra_range'}) {
		last unless (@query_big);
		my $tmp = shift @query_big;
		$uid_tmp_lst .= "$tmp,";
		push @query_list_1day, $tmp;
...
#	}
	...
	if ($day == 0){
		...
		if ($simulation_file) {
		    ...
		    ($tmp_result,$connection_error) = &make_HTTP($PARAM{'retrieve_URL'},$docstring,$alias);
		}
		...
		while ( ( $tmp_result =~ /Server Error/s
		    ...
		    my $time_now = localtime;
		    print "\nWARNING at $time_now on $ip: $text, starting retry $retry_local...\n";
		    warn "WARNING at $time_now $PARAM{'id'} on $ip: $text, retry $retry_local for query $uidstring at second visit\n";
		    print "sleeping for $PARAM{'break'} seconds...\n";
		    sleep($PARAM{'break'});	
		    $time_1 = time if ($PARAM{'log_queries'});
		    ($tmp_result,$connection_error) = &make_HTTP($PARAM{'retrieve_URL'},$docstring,$alias);
...
#		&update_output;
		...
		unless ($tmp_result =~ /<\/eSummaryResult>/si) {
		    ...
		    $tmp_result = "<BR><CENTER><BIG><B>Received only partial results for this query, please <a href=\"$prefix$uidstring$suffix\">download results manually</a>. </B></BIG></CENTER><BR>\n";
		    if ($PARAM{'quickstart'}) {
			$tmp_result .= "\n<CENTER><BIG><B>PubCrawler advises:</B>\n<B>\&rarr; <A HREF=\"${HTML}quickstart.html\">restart your jobs manually!</A></B></BIG></CENTER><BR>\n" ;
			$net_failure = 1;
			$advise = 1;
		    }
		    $second_visit_result .= $tmp_result;
		} else {
		    # parse XML results
...
#		    my $link_out = "\n<A HREF=\"$summary_link\&db=$retrieve_db{$search_type}\&dispmax=$counter2\&list_uids=$uidstring\">Retrieve this group of hits</A><BR>";
		    ...
		    my $link_out = &retrieve_this_group($prefix, $uidstring, $suffix);
		    ...
		    if ($mail_features{'entrez_links'}) {
#			$insert_text = "\n<A HREF=\"$summary_link\&db=$retrieve_db{$search_type}\&dispmax=$counter2\&list_uids=$uidstring\">Retrieve this group of hits</A><BR>";
			$insert_text = &retrieve_this_group($prefix, $uidstring, $suffix);

			unless ($mail_features{'html'}) {			    
#			    $insert_text = "\nRetrieval of above hits: $summary_link\&db=$retrieve_db{$search_type}\&dispmax=$counter2\&list_uids=$uidstring";
			    $insert_text = "\nRetrieval of above hits: $prefix$uidstring$suffix";
			}
		    }
...
#		$button = "\nMORE: $day-day-old records for \'$alias\'  <A HREF=\"$summary_link\&db=$retrieve_db{$search_type}\&dispmax=$retrieve_number\&list_uids=$uidstring\">(";
		$button = "\nMORE: $day-day-old records for \'$alias\'  <A HREF=\"$prefix$uidstring$suffix\">(";
		$button .= ''
		    .($#query_big >= 0?"1 - ":"")
...
#		$button =~ s/<A\&nbsp;HREF/<A HREF/g;
		...
		if ($mail_features{'entrez_links'}) {
			...
			$button = "\nMORE: $day-day-old records for \'$alias\'("
			    .($#query_big >= 0?"1 - ":"");
#			    $button .= "$number_of_1day_records),  $summary_link\&db=$retrieve_db{$search_type}\&dispmax=$retrieve_number\&list_uids=$uidstring";
			$button .= "$number_of_1day_records),  $prefix$uidstring$suffix";
		    } 
		} else {
		    $button = "\nMORE: $day-day-old records for \'$alias\'("
			    .($#query_big >= 0 ? "1 - " : '')
...
#	        $button = "  <A HREF=\"$summary_link\&db=$retrieve_db{$search_type}\&dispmax=$retrieve_number\&list_uids=$uidstring\">($counter - $number_of_1day_records)</A>";
	        $button = "  <A HREF=\"$prefix$uidstring$suffix\">($counter - $number_of_1day_records)</A>";
...
#			$button_mail = " ($counter - $number_of_1day_records): $summary_link\&db=$retrieve_db{$search_type}\&dispmax=$retrieve_number\&list_uids=$uidstring\n";
			$button_mail = " ($counter - $number_of_1day_records): $prefix$uidstring$suffix\n";
		    }
		} else {
...
#	        $button = "$more_text for \'$alias\'  <A HREF=\"$summary_link\&db=$retrieve_db{$search_type}\&dispmax=$retrieve_number\&list_uids=$uidstring\">($counter - $number_of_1day_records)</A>";
	        $button = "$more_text for \'$alias\'  <A HREF=\"$prefix$uidstring$suffix\">($counter - $number_of_1day_records)</A>";
...
#			$button_mail = "$more_text_mail for \'$alias\'  ($counter - $number_of_1day_records): $summary_link\&db=$retrieve_db{$search_type}\&dispmax=$retrieve_number\&list_uids=$uidstring\n";
			$button_mail = "$more_text_mail for \'$alias\'  ($counter - $number_of_1day_records): $prefix$uidstring$suffix\n";
		    }
		} else {
...
For your convenience, the following form allows to carry out searches at the US National Library of Medicine (NLM) and PubMed through <A HREF=\"https://www.ncbi.nlm.nih.gov/\">NCBI</A>:
<table>
<tr>
...
for
<input name=\"term\" size=\"45\" type=\"TEXT\" value=\"<insert search and click on 'Go' (do not hit 'Enter')>\">						
...
<TR>
<TD><CENTER>
PubCrawler is hosted by the <a href=\"http://www.tcd.ie/Genetics\">Genetics Department</a>, Trinity College Dublin, Ireland. It was developed by <a href=\"http://bioinf.gen.tcd.ie/~khokamp\">Karsten Hokamp</a> and <A HREF=\"http://wolfe.ucd.ie\">Ken Wolfe</A> (now UCD).
...
<BR>All records stem from the National Library of Medicine and were retrieved by PubCrawler through NCBI\'s <A HREF=\"https://eutils.ncbi.nlm.nih.gov/entrez/query/static/eutils_help.html\">E-utils tools</A>.
<BR>Please take a look at <A HREF=\"https://eutils.ncbi.nlm.nih.gov/About/disclaimer.html\">NCBI\'s Disclaimer</A> for more information about their disclaimers and copyrights.
</CENTER>
</TD>
</TR>
</TABLE>
...
PubCrawler is hosted by the Genetics Department, Trinity College Dublin, Ireland. 
It was developed by Karsten Hokamp and Ken Wolfe (now UCD).
...
sub retrieve_this_group {
    my $prefix = shift;
    my $uidstring = shift;
    my $suffix = shift;
    my $length = length("$prefix$uidstring$suffix");
    if ($length <= 4080) {
	return "\n<A HREF=\"$prefix$uidstring$suffix\">Retrieve this group of hits</A><BR>";
    } else {
	my @tmp = split /,/, $uidstring;
	my $counter = 1;
	my $out = "\nRetrieve this group of hits:";
	while (@tmp) {
	    my $uid_tmp_lst = '';
	    my $hits = 0;
	    while (length($prefix.$uid_tmp_lst.$suffix) <= 4072) {
	    ...
	    $out .= "&nbsp;&nbsp;<A HREF=\"$prefix$uid_tmp_lst$suffix\">batch $counter ($hits hits)</A>";
	    $counter++;
	}
	$out .= "<br>";
	return $out;
...
#	$request_URL eq $PARAM{'neighbour_URL'}) {
    ...
    if ($res->is_success) {
	if ($res->content =~ /Authentication Required/
	    or
	    $res->content =~ /TCD Proxy Authentication/
	    or
	    $res->content =~ /Firewall Authentication/) {
            my $stime = 0;
	    my $tag = 'Authentication';
	    $connection_error = 1;
		    ...
		    if ($stime > 300) {
			warn "No $tag after $stime seconds for $PARAM{'id'}, $alias on ".(localtime)." on '$hostname' ($ip) - cancelling\n";
			last;
		    } 
		    sleep 10;
		    $stime += 10;
		    ...
		    $connection_error = 0;
                    warn "$tag after $stime seconds for $PARAM{'id'}, $alias on ".(localtime)." on '$hostname' ($ip)\n";
		}
	    }
	}
	#print "\n res content \n";
	...
	$connection_error = 1;
	if ($res->content =~ /Server closed connection without sending any data back/
	    or
	    $res->content =~ /Authentication Required/
	    or
	    $res->content =~ /Firewall Authentication/) {
	    my $stime = 0;
	    my $tag = 'Server response';
	    if ($res->content =~ /Authentication Required/
		or
		$res->content =~ /Firewall Authentication/) {
		$tag = 'Authentication';
    ...
    if ($rescontent =~ /TCD Proxy Authentication/i
	or
	$rescontent =~ /Firewall Authentication/i) {
	warn "Firewall problem:\n$rescontent\n\n";
	$rescontent = '';
	$connection_error = 1;
	sleep 10;
    }
...
sub read_config {
  ...
  WHILE:while (<CONFIG>){
        ...
        $line++;
	unless (/^\s*$known_searchtypes\s+(.*)/) {
	    # queries might contain hash mark in alias or search term!
...
and the GenBank-database at NCBI at https://www.ncbi.nlm.nih.gov/ .
...
available options: 
'DocSum','Brief','Abstract','AbstractPlus','Citation','MEDLINE','ASN.1','ExternalLink','GenBank','FASTA'
...
The definition of the search terms is the same as given by I<Entrez Search System>. Please look up their site for more information ( https://www.ncbi.nlm.nih.gov/Entrez/entrezhelp.html#ComplexExpression )
...
check out PubCrawlers web-page at $home_link

If none of this advice helps you should send an e-mail with a detailed 
description of the problem to pubcrawlerinfo@gmail.com

=head1 DOCUMENTATION

=head1 AUTHOR

Original author:  Prof. Ken H. Wolfe, kenneth.wolfe@ucd.ie

Second author: Dr. Karsten Hokamp, kahokamp@tcd.ie

Both worked originally in the Department of Genetics, Trinity College, Dublin
but Ken moved to University College Dublin in 2013.
...
This program has a home page at $home_link .

=head1 LAST MODIFIED

$Id: pubcrawler.pl,v 2.36 2016/09/10 13:25:24 pubcrawl Exp pubcrawl $
